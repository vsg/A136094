 ...
 * --------------------------------------------------------------------
 * MODIFICATIONS:
 * - Changed package to oeis.a136094.util
 * - Renamed class from HashMap to MemoryEfficientHashMap
 * 
 * Modified by Vitaliy Garnashevich, 2026
 * --------------------------------------------------------------------
 */
...
public class MemoryEfficientHashMap<K, V> implements Map<K, V>, Serializable {
  ...
  private class EntrySet extends AbstractSet<Entry<K, V>> {
    ...
    public boolean add(Entry<K, V> entry) {
      boolean result = !MemoryEfficientHashMap.this.containsKey(entry.getKey());
      MemoryEfficientHashMap.this.put(entry.getKey(), entry.getValue());
      return result;
    }
    @Override
    public boolean addAll(Collection<? extends Entry<K, V>> c) {
      MemoryEfficientHashMap.this.resizeForJoin(c.size());
    ...
    public boolean contains(Object o) {
      ...
      V value = MemoryEfficientHashMap.this.get(entry.getKey());
  ...
  public MemoryEfficientHashMap(boolean stableWriteObject) {
    this.stableWriteObject = stableWriteObject;
    initTable(INITIAL_TABLE_SIZE);
  }
  public MemoryEfficientHashMap(Map<? extends K, ? extends V> m) {
